<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="date" label="值日日期"></el-table-column>
    ...
    <el-dialog title="值日信息" :visible.sync="dialogFormVisible" width="30%" >
...
export default {
  ...
  methods: {
    ...
    ,
    changeEnable(row) {
      this.request.post("/duty/save", row).then(res => {
        if (res.code === '200') {
    ...
    del(dutyid) {
      this.request.delete("/duty/" + dutyid).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/duty/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    save() {
      this.request.post("/duty", this.form).then(res => {
        if (res.code === '200') {
