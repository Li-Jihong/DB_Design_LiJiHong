<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="paymentid" label="交款号" width="80"></el-table-column>
      ...
      <el-table-column label="操作" width="280" align="center">
        <template slot-scope="scope">
          <!--          <el-button type="primary" @click="selectpayment(scope.row.id)">选课</el-button>-->
    ...
    <el-dialog title="付款信息" :visible.sync="dialogFormVisible" width="30%" >
      <el-form label-width="80px" size="small">
        <el-form-item label="付款号">
          <el-input v-model="form.paymentid" autocomplete="off"></el-input>
        </el-form-item>
        ...
        <el-form-item label="付款日期">
          <el-input v-model="form.paymentdate" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    changeEnable(row) {
      this.request.post("/payment/save", row).then(res => {
        if (res.code === '200') {
    ...
    del(studentid) {
      this.request.delete("/payment/" + studentid).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/payment/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    save() {
      this.request.post("/payment", this.form).then(res => {
        if (res.code === '200') {
